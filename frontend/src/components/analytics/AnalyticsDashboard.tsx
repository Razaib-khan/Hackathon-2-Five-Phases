...
export function AnalyticsDashboard({ userId, className = '' }: AnalyticsDashboardProps) {
  const { analytics, streak, loading, error, fetchAnalytics, fetchStreak } = useAnalytics()
  const [period, setPeriod] = useState<'week' | 'month' | 'year' | 'all'>('month')

  // Fetch analytics data on mount and when period changes
  useEffect(() => {
    if (userId) {
      fetchAnalytics(userId, period)
      fetchStreak(userId)
    }
  }, [userId, period, fetchAnalytics, fetchStreak])
    ...
    <div className={`${className}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <CompletionTrendChart
          data={analytics.completion_trend}
        ...
        <TimeTrackingChart
          ...
          totalTimeSpent={analytics.total_time_spent}
          averageCompletionTime={analytics.average_completion_time}
        />

        {/* Streak Calendar */}
        <StreakCalendar
          completionData={analytics.completion_trend}
